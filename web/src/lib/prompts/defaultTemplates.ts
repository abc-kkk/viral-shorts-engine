...
【剧本硬性要求（必须严格遵守）】
1. **支持多场景与灵活对话**：解除单一物理场景限制。如果故事需要转换地点或发生较长的时间跨度，可以自由跳转场景。台词长短不限，可以根据剧情需要说出长句或短句互怼，由你自由发挥。
...
【分镜拆解规则】：
1. 剧本长度自由：支持长篇或多幕结构。必须将剧本完整无损地拆解出所有的镜头，绝对不要遗漏任何台词。可以自然地把长段对白放入一个分镜中。
2. 对白必须紧凑有力，保留所有的搞笑包袱和反转。
3. 如有时间流逝（如"第二天"、"三小时后"）或场景切换，必须输出单独的字卡行（speaker: "字卡"）。
...
- 服饰设计 (差异化要求)： 除非文案明确标注“衣服破损”，否则服装完整无破损。
  - 上身服装: [根据人设设计：具体的款式、3D布料材质、颜色、剪裁细节，严禁默认西装]。
  - 下身服装: [根据人设设计：具体的裤装/裙装款式、3D材质贴图、颜色细节]。
  - 鞋履配饰: [匹配的鞋子；除非文案明确要求，否则不加冗余首饰，拒绝跨性别装饰]。
- 负向提示词要求：提示词结尾必须加上 "no realistic, no photo, no photograph, no真人, 禁止写实, 禁止真人照片"。
