...
【规范2: imagePrompt 三段式书写结构（静态画面描述铁律）】
imagePrompt 和 startImagePrompt 是给生图模型的**静态画面描述**。你必须严格按照以下三段式顺序书写：

**【第一段：场景与资产锚定】**
必须以 {@{{sceneLocationToken}}} 和 {@角色名} 开头，锁死背景和人物参考图。简要交代人物在场景中的相对位置（如"画面左侧"、"画面右侧"）。

**【第二段：人物终态定格（极其关键！）】**
只描述人物在该镜头**结束瞬间**的静态姿态、表情、肢体位置和道具状态。这是一张静止的照片，按下了暂停键的那一帧！
- 严禁写动态过程词（如"正在推卡"、"缓慢伸手"、"转头看向"）！这些是 videoPrompt 的职责！
- 必须写终态定格词（如"手已平放在玻璃窗台上，黑卡静止于窗台中央"、"嘴角微微上扬，露出满意的笑容"）。

**【第三段：摄影级环境与构图】**
用极其细腻的笔触描写以下维度，这是你的核心价值：
- **空间布局（极简原则）**：严禁添加复杂的家具或障碍物。只描述最基础的家具相对位置。保持背景极度干净和留白！
...
3. **口播同步与声线锚定（极其关键！）**：如果角色有台词，你必须做到以下所有点：
   ...
   - 强调"动作幅度极小"、"神情自然松弛"。
   - **Veo 3.1 原生音频引擎**：Veo 3.1 在生成视频时会通过联合扩散过程原生合成对话语音与环境音效。你写的声线描述就是它的音色调控指令。因此声线描述不仅要写性别和音色，还要融入当前情绪（如"用压低了声音、带有一丝颤抖的年轻女声说道"）。
...
5. **极速聚焦说话者（解决“认错人”最高优先级！）**：AI 视频模型经常会搞错是谁在说话。解决这个问题的唯一方法是：**在视频开头，让镜头迅速锁定/切换到说话人的脸上，立刻开口说话，绝对不要有任何多余的前置动作！**
   ...
   - 例如："视频开始，镜头迅速推向画面左侧的布偶猫，形成侧脸特写。布偶猫立刻嘴巴轻微开合，说道：'没钱才来取啊！' 说完后，布偶猫缓缓将黑卡推向防弹玻璃，镜头随之稍微拉开，展现人物端坐于沙发之上，身体正对镜头，上半身自然前倾。背景为夜晚真实卧室场景，沙发右侧优先贴近墙面，其上放置一个体积显著偏大的毛绒玩偶，摆放不对称。采用写实摄影风格与纪录感生活影像基调，主光源为真实夜晚灯光，背景轻微虚化，坚守纪实质感。"
...
{
  "startImagePrompt": "在{@{{sceneLocationToken}}}中，{@小雪} 位于画面中央，端坐于沙发之上。人物保持自然静坐姿态，双手自然垂放于膝上，神情松弛平和，嘴唇微微闭合，眼神柔和地直视前方，无任何多余动作或道具。正面构图，镜头位于人物正前方与眼睛平齐高度，近景构图，脸部在画面中占比偏大。背景为夜晚居家卧室，沙发横向贯穿画面，右侧紧邻墙面，沙发上放置一个体积偏大的毛绒玩偶，摆放不对称。整体色调偏冷中性，主光源为白色中性灯光，光线分布略不均匀，带有轻微自然阴影。背景呈轻微虚化效果，写实纪录风格。",
  "imagePrompt": "在{@{{sceneLocationToken}}}中，{@小雪} 位于画面中央，端坐于沙发之上。上半身自然前倾，右手已举起手机，手机屏幕正对镜头方向，屏幕显示锁屏界面，表情松弛自然，眼神看向手机屏幕方向。正面构图，镜头位于人物正前方与眼睛平齐高度，近景构图，脸部在画面中占比偏大。背景为夜晚居家卧室，沙发横向贯穿画面，右侧紧邻墙面，沙发上放置一个体积偏大的毛绒玩偶。整体色调偏冷中性，主光源为白色中性灯光，光线略不均匀，背景轻微虚化。写实纪录风，生活影像质感。",
...
{
  "imagePrompt": "在{@银行柜台}中，画面左侧的 {@布偶猫} 站在防弹玻璃外，画面右侧玻璃内的 {@柯基柜员} 坐在柜台后。布偶猫身体侧向右边，右手已平放在玻璃窗台上，一张黑色银行卡静止于窗台中央位置，脸部朝向画面右侧，表情坚定冷酷。柯基柜员双手放在键盘上，低头注视着窗台上的黑卡，眉头微微皱起。过肩镜头，从柯基的右后方看向布偶猫，布偶猫的侧脸特写占满画面左半部分。整体色调明亮温馨，主光源为顶部白色银行灯管，光线分布均匀偏冷。写实摄影风格，毛发质感清晰可辨。",
