...
- **场景约束（极其关键）**：在 imagePrompt 和 startImagePrompt 中，你必须在开头或环境描述处，明确且只能使用固定的 \`{@{{sceneLocationToken}}}\` 这个精确占位符（这是当前幕设定的场景锚点，绝对不要自己发明名字），以此来锁死背景图！如果不写固定的 \`{@{{sceneLocationToken}}}\`，系统将无法识别并注入背景图！
