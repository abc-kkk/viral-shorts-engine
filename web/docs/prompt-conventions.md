...
**后端自动生成文件时，必须严格遵守以下标识符，任何开发者不得乱改此规则**：
...
- **现象**：新增了 `sceneLocationImage` 这一资产类型，但生图后无法回显到前端。
- **根因**：新 `targetType` 只在后端设置了，遗漏了其他消费端。
- **铁律**：每次新增一个 `targetType`，必须同步修改相关前后端代码，特别是：
  1. `lib/types.ts` 中的 `TargetType` 联合类型和 `TARGET_TYPE_CONFIG` 注册表
  2. `app/api/generate-assets/route.ts` 或 `lib/assetUrl.ts` 中的文件命名规则
  - 缺一个都会导致命名异常或数据丢失！
...
- **现象**：系统生图显示"✅ 成功"，磁盘上文件也已更新，但 Studio 工作台始终显示昨天的旧图。用户硬刷新也没用。
- **根因**：
...
- **修复**：
  1. 返回的 URL 追加 `?v=${Date.now()}` 缓存破坏参数。
...
- **铁律**：
  ...
  - **涉及文件**：`serve/[...path]/route.ts`、`useProjectStore.ts`
