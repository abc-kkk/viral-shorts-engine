...
> 3. `architecture.md`：**核心业务架构**，涵盖提示词模板引擎、布局预设系统、全局状态解耦及 Chrome 扩展通信机制。
> 4. `database-migration.md`：**数据库映射避坑指南**，详细记录 Prisma 到 Drizzle 的迁移与 `ProjectState` 字段落地映射铁律。
> 5. `desktop-packaging.md`：**桌面端打包与环境指南**，解决 Electron C++ ABI 失配、网络代理死锁以及 Node.js 僵尸进程等底层系统坑点。
...
7. **客户端全自动热更新**：内置全静默增量热更新（Differential Updates）与可视化进度条。同时底层搭载数据库自愈机制，彻底解决跨平台环境异常导致的数据损毁。
